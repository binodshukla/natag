							...
							<?php if(have_posts()):while(have_posts()):the_post(); ?>
									...
									<?php $postid = get_the_ID();
									 ...
									 ?>
										...
										<div style="float:left;width:475px;min-height:200px;">
                                      <div sty><?php the_content(); ?></div>
...
$CustomerInfo = $wpdb->get_results("SELECT * FROM wp_posts Where post_type=products");
...

 {
   ...
   echo $customer_name;
	...
	$i=0;
	while(array_count_values($q)>i)
	{
		  
		$q1=$wpdb->get_results('SELECT ID FROM wp_posts Where post_title=$q[i]');    
		echo $q[i];
		...
		while(array_count_values($q1)>j)
		{
			...
			//echo $q[i];
			//echo '<p>Total miles is {$submenu}</p>';
			$j++;
