 ...
 * @package WordPress
 * @subpackage natag
 * Template Name:List of Equipment
 */
...
<form name="home" action="<?php echo get_option('siteurl')?>/?page_id=<?php echo $_REQUEST['page_id']?>" method="get">
                                ...
                                <?php
                                            $myrows = $wpdb->get_results( "SELECT wp_posts.ID,wp_posts.post_title,wp_posts.post_status,wp_posts.post_name,wp_posts.post_date from wp_posts where wp_posts.post_type='equipment' and (wp_posts.post_status='sent' or wp_posts.post_status='publish') order by wp_posts.ID DESC " );
                                            $ct=1;
